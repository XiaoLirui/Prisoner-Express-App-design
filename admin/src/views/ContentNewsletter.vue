...
const allItems = ref(Object.entries(fixtures).map(([key, value]) => ({
  key,
...
watch(() => route.params.key, (newKey) => {
  items.value = allItems.value.filter(item => item.type === newKey);
}, { immediate: true });
...
function updateTable() {
  ...
  items.value = [newItem, ...items.value]; // Prepend new item in a way that Vue recognizes
  tableModified.value = true;
    ...
    <b-row>
      ...
      <b-col cols="12" md="8" lg="9" class="main-content p-4">
        <div class="d-flex justify-content-between align-items-center mb-2">
          <h2 style="text-transform: capitalize;">App Content/{{$route.params.key}}</h2>
        </div>
        ...
        <div class="d-flex justify-content-between mb-4">
          <div class="left-actions">
            ...
            <b-button disabled class="mx-2" v-else>Publish App</b-button>
          ...
          <b-modal v-model="upload" hide-footer class="mb-3">
            Upload File to <span style="text-transform: capitalize;">{{$route.params.key}}</span>
